...
def listar_reservas():
    ...
    description: |
      Lista todas las reservas con posibilidad de filtrar por sucursal, cliente, estado y rango de fechas.
      Solo retorna reservas de la sucursal del usuario autenticado (multi-tenant).
      Todos los parámetros en el body son OPCIONALES - envía solo los que necesites para filtrar.
    parameters:
      - in: body
        name: body
        required: false
        description: "Filtros opcionales - todos los campos son opcionales"
        schema:
          type: object
          properties:
            sucursal_id:
            ...
            cliente_id:
              type: integer
              description: "ID de cliente para filtrar (opcional)"
              example: 2
            estatus:
              type: integer
              description: "Estado de la reserva (opcional). Valores: 1=Programada, 2=EnCurso, 3=Completada, 4=NoShow, 5=Cancelada"
              example: 1
            fecha_desde:
              type: string
              format: "yyyy-mm-dd hh:mm:ss"
              description: "Inicio del rango de fechas en formato yyyy-mm-dd hh:mm:ss (zona horaria América/México_City, opcional)"
              example: "2025-11-22 00:00:00"
            fecha_hasta:
              type: string
              format: "yyyy-mm-dd hh:mm:ss"
              description: "Fin del rango de fechas en formato yyyy-mm-dd hh:mm:ss (zona horaria América/México_City, opcional)"
              example: "2025-11-22 23:59:59"
    responses:
      200:
    ...
    x-code-samples:
      - lang: curl
        source: |
          # Ejemplo 1: Listar TODAS las reservas (sin filtros - body vacío)
          curl -X POST http://localhost:5000/api/reservas/listar \\
            -H "Authorization: Bearer YOUR_JWT_TOKEN" \\
            -H "Content-Type: application/json" \\
            -d '{}'
      - lang: curl
        source: |
          # Ejemplo 2: Filtrar por sucursal solamente
          curl -X POST http://localhost:5000/api/reservas/listar \\
            -H "Authorization: Bearer YOUR_JWT_TOKEN" \\
            -H "Content-Type: application/json" \\
            -d '{
      ...
      - lang: curl
        source: |
          # Ejemplo 3: Filtrar por sucursal y estado
          curl -X POST http://localhost:5000/api/reservas/listar \\
            -H "Authorization: Bearer YOUR_JWT_TOKEN" \\
            -H "Content-Type: application/json" \\
            -d '{
      ...
      - lang: curl
        source: |
          # Ejemplo 4: Filtrar por cliente en sucursal específica
          curl -X POST http://localhost:5000/api/reservas/listar \\
            -H "Authorization: Bearer YOUR_JWT_TOKEN" \\
            -H "Content-Type: application/json" \\
            -d '{
      ...
      - lang: curl
        source: |
          # Ejemplo 5: Filtrar por rango de fechas y estado
          curl -X POST http://localhost:5000/api/reservas/listar \\
            -H "Authorization: Bearer YOUR_JWT_TOKEN" \\
            -H "Content-Type: application/json" \\
            -d '{
      ...
      - lang: curl
        source: |
          # Ejemplo 6: Filtro completo (todos los parámetros)
          curl -X POST http://localhost:5000/api/reservas/listar \\
            -H "Authorization: Bearer YOUR_JWT_TOKEN" \\
            -H "Content-Type: application/json" \\
            -d '{
              ...
              "fecha_hasta": "2025-11-23 23:59:59"
            }'
    x-important-notes: |
      NOTAS IMPORTANTES:
      1. TODOS LOS CAMPOS EN EL BODY SON OPCIONALES
      2. Si no envías un campo, no se filtra por ese criterio
      3. Los campos que envíes se aplicarán como AND (filtro intersección)
      4. Si envías {} (body vacío), retorna TODAS las reservas
      5. Las fechas deben estar en formato: yyyy-mm-dd hh:mm:ss
      6. La zona horaria es América/México_City
    """
    try:
        # Usuario autenticado
        current_user = get_jwt_identity()
        ...
        try:
            ...
            estatus = int(data.get('estatus')) if data.get('estatus') else None
        except (ValueError, TypeError):
            return jsonify({"error": "Parámetros inválidos", "detalles": "sucursal_id, cliente_id y estatus deben ser números enteros"}), 400
